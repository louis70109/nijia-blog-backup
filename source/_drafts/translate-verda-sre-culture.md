...
同時我們現在鼓勵用戶把專案從 Baremetal 轉到 VM 上並有效地使用 VM，降低一些具有嚴格性能要求與資源隔離 VM 的使用成本。
