...
注意到在切換 Rich Menu 給用戶看之前，必須有 4 個請求，當然請求越多，延遲也越高。
...
注意：LINE 服務器收到請求後，除了為用戶切換 Rich Menu 外，同時還會以 Postback 事件的形式向 Bot 應用發送 Wehook。
...
# 1. 為 Type A 和 B 準備豐富的菜單

要為 A 和 B 的 Rich Menu 創建圖像，請按照以下文章的第 1 步操作。
...
在這一步中，取上一步得到的 JSON，修改新的 Action 如下。
...
LINE 發布的 Richmenu Switch Action 從用戶的角度幫助了 Rich Menu 的實時切換，同時也減少了開發者的工作量。告訴我是誰在這裡讀的。不用不知道說啥哈哈哈哈
